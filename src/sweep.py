#!/usr/bin/env python3
"""
sweep.py — W&B hyperparameter sweep for DA6401 Assignment-1 (Section 2.7).

Run from inside src/:
    python sweep.py --project da6401-assignment1 --count 100

This creates a W&B sweep with 100 runs varying all key hyperparameters
and produces the Parallel Coordinates plot required for the report.
"""
...
SWEEP_CONFIG = {
    "method": "bayes",           # Bayesian search (smarter than random for 100 runs)
    "metric": {
        "name": "val_accuracy",
        "goal": "maximize",
    },
...
# ---------------------------------------------------------------------------
# Sweep agent training function
# ---------------------------------------------------------------------------

def sweep_train():
    """Called by W&B agent for each sweep run."""
    import wandb
...
def main():
    p = argparse.ArgumentParser(description="W&B sweep for DA6401 Assignment-1")
